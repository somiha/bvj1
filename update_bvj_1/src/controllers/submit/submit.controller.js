...
const doSubmit = async (req, res) => {
  try {
    let { title, type, abstract, keywords } = req.body;
    ...
    const insert_query = (paper_id) =>
      `INSERT INTO paper_info (id, user_id, checkpoint, title, type, abstract, keywords ) VALUES (${paper_id}, ${user_id}, ${ADD_AUTHOR}, '${title}', '${type}', '${abstract}', '${keywords}' )`;
...
const renderAddAuthor = async (req, res) => {
  ...
  try {
    ...
    var alreadyAddedAuthors = readData(
...
const doAddAuthor = async (req, res) => {
  const { paper_id } = req.params;
  try {
    var authorList = readData(
      "SELECT id FROM paper_author WHERE paper_id = " + paper_id
    );
    var corresponding_author = readData(
    ...
    if (authorList[0]) {
      if (corresponding_author[0]) {
        ...
        res.redirect(
          `/submit/${paper_id}/add-author?msg=${encodeURIComponent(
  ...
  try {
    ...
    if (!result[0]) {
      ...
      if (result[0].checkpoint !== UPLOAD_FILE) {
        ...
        hasError = true;
        errorMessage =
          "You don't have permission to upload file for this paper";
      }
    }
  } catch (error) {
    console.log(error);
...
const doUploadFile = async (req, res) => {
  try {
    ...
    if (!result[0]) {
      ...
      deleteUploadedFile(req.file.filename);
      res.status(403).json({
        message: "You don't have permission to upload file for this paper",
      });
    }
    const { file } = req;
...
async function apiDoAddAuthor(req, res) {
  try {
    ...
    // let { id, first_name, middle_name, last_name, email } = req.body;

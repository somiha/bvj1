...
const { ADD_AUTHOR, UPLOAD_FILE, SUBMITTED, COMPLETED } = require("../../enums/paperCheckpoints");
...
const doAddOldArticle = async (req, res) => {
  try {
    ...
    createUpdateDelete(insert_query, dataObj);
...
const renderAddAuthorAdmin = async (req, res) => {
  ...
  try { 
    ...
    res.render('admin-panel/add-old-article/add-author', {
      ...
      isAdmin: req.isAdmin,
      paperId: req.params.paper_id, alreadyAddedAuthors, msg
    })
  } catch(error) { 
    console.log(error)
  }
  ...
  try {
    ...
    if(authorList[0]){
      if(corresponding_author[0]){
        ...
        res.redirect(`/admin_panel/add_old_article/${paper_id}/add-author?msg=${encodeURIComponent('Please Add a Corresponding Author For This Paper.')}`);
      }
    } else {
      res.redirect(`/admin_panel/add_old_article/${paper_id}/add-author?msg=${encodeURIComponent('Please Add One or More Author For This Paper.')}`);
    }
  } catch(error) { 
    console.log(error);
  }

...
const {
  TranasctionTwo,
  TranasctionThree,
} = require("../../helper/dbTransactionQuery");
...
async function apiSaveReview(req, res) {
  try {
    let { paper_id, files_url, comment } = req.body;
  ...
  try {
    ...
    if (review.length === 0) {
      ...
      const query2 = `UPDATE paper_info SET checkpoint = ${
        checkpoint === IN_REVIEW ? REVIEW_DONE : REVISION_DONE
      }, reviewer_id = ${req.decode_data.id} WHERE id = ${paper_id}`;
      const query3 = `UPDATE reviewers SET total_review = total_review + 1 WHERE id = ${req.decode_data.id}`;
      await TranasctionThree(query1, query2, query3);
    }
    res.json({
      success: true,

...
const doLogin = async (req, res) => {
  try { 
    ...
    if(email == process.env.ADMIN_EMAIL && password == process.env.ADMIN_PASSWORD) {
      const token = jwt.sign({ email: process.env.ADMIN_EMAIL, role: 'admin' }, process.env.JWT_ADMIN_SECRET, { expiresIn: '24h' });
      ...
      if (getData.length === 0) {
        ...
        if (bcrypt.compareSync(password, getData[0].password)) {
          const token = jwt.sign({ ...getData[0] }, process.env.JWT_SECRET, { expiresIn: '24h' });
...
const doResetPass = async (req, res) => {
  ...
  if(password !== c_password) {
    ...
    try { 
      ...
      if (getData.length > 0) {
        if (token === getData[0].remember_key) {
          const update_query = `UPDATE users SET password = '${dcryptPass(password)}' WHERE email = '${email}'`;
          const forgot_pass_key = `UPDATE users SET remember_key = '${require('crypto').randomBytes(20).toString('hex')}' WHERE email = '${email}'`;
          createUpdateDelete(update_query);
          createUpdateDelete(forgot_pass_key);
          res.render('login/reset-pass', {
            path: '/login',

...
const processReg = async (first_name, middle_name, last_name, email, password) => {
  if(first_name && last_name && email && password){
    const query = `INSERT INTO users SET ?`;
    const value = { first_name, last_name, middle_name, email, password: dcryptPass(password), role: AUTHOR, remember_key: forgotPassKey.encode() };
    try { 
...
const doRegister= async (req, res) => {
  ...
  if(await checkEmailExist(email)){
    res.send(await processReg(first_name, middle_name, last_name, email, password));
  }else {

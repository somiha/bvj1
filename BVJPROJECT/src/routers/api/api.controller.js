const { reviewer_mail, author_paper_update } = require("../../config/nodemailer.config");
...
async function apiDoUpdateReviewer(req, res){
  try {
    ...
    createUpdateDelete(query, value);
    res.json({
      success: true,
...
async function apiSearchReviewer(req, res){
  ...
  if(!search_query) return res.status(400).json({ success: false, message: 'Search query is required' });
...
async function apiAssignReviewer(req, res){
  try { 
    ...
    createUpdateDelete(insert_query, value);
    const result = await readData(query);
    reviewer_mail(result[0].first_name, reviewerId, result[0].email)
    res.send({
      success: true,
...
const apiAddVolume = async (req, res) => {
  try {
    const position = await readData(`SELECT IFNULL(MAX(position), -1) as position FROM volumes`);
    const index = parseInt(position[0].position);
    const query = `INSERT INTO volumes SET title = '${req.body.title}', position = ${index + 1};`;
    createUpdateDelete(query);
    res.json({
      success: true,
...
async function apiMovePosition(req, res){
  try {
    ...
    const maxPosition = await readData(`SELECT IFNULL(MAX(position), 0) as position FROM volumes`);
    ...
    if(method === 0){
      ...
      const query2 = `UPDATE volumes SET position = position + 1 WHERE position = ${position - 1} AND id <> ${id}`;
...
async function apiAddPaper(req, res){
  try { 
    ...
    if(!(paper_id.length > 0) || !(volume_id.length > 0) || !(page.length > 0)) return res.status(400).json({ success: false, message: 'Paper id and volume id is required' });
...
async function apiUpdateArticleAuthor(req, res){
  try { 
    ...
    const insert_query = `INSERT INTO paper_author (paper_id, is_corresponding, first_name, middle_name, last_name, email, affiliation, country) VALUES (${paper_id}, ${is_corresponding ? 1 : 0}, '${first_name}', '${middle_name}', '${last_name}', '${email}', '${affiliation}', '${country}');`;
...
async function apiUpdateArticlePaperInfo(req, res){
  try {
    ...
    createUpdateDelete(update_query, values);
    res.send({
      success: true,
...
async function apiAuthorSearchAndUpdate(req, res){
  try { 
    ...
    const result = await readData(select_query);
    if(result.length > 0){
      const { paper_id, first_name, middle_name, last_name, email, affiliation, country, is_corresponding } = result[0];
...
async function apiUploadFile(req, res){
  try { 
    ...
    const query2 = `UPDATE paper_file SET file_url = '${file.filename}', file_type = '${fileType[fileType.length - 1]}' WHERE paper_id = ${paper_id}`;
    createUpdateDelete(query2);
...
module.exports = { 
  apiDoUpdateReviewer, apiSearchReviewer, apiReviewerList, apiAssignReviewer, apiSearchPaper, apiAddVolume, apiVolumeList, apiMovePosition, apiEditVolume, apiDeleteVolume, apiAddPaper, apiAuthorSearch,
  apiPaperCompleted, apiUpdateArticleAuthor, apiUpdateArticlePaperInfo, apiAuthorSearchAndUpdate, apiUploadFile, apiAdminDeletePaper, apiGetReview, apiAuthorDeletePaper, apiAdminRejectPaper,
  apiDeleteAuthor, apiSearchAuthor
};